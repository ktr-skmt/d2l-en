...
Since `x` is a vector of length 4, `nd.dot` will perform inner product and therefore `y` is a scalar. Next, we can automatically find the gradient of all the inputs by calling the `backward` function.
